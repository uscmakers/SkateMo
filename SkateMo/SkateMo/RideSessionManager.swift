...
final class RideSessionManager: ObservableObject {
    ...
    init(
        boardBLEManager: BoardBLEManager = BoardBLEManager(),
        ...
        self.boardBLEManager = boardBLEManager
    ...
    private func configureServices() {
        locationManager.onLocationUpdate = { [weak self] location in
            ...
            self.navigationManager.updateLocation(
                location,
                headingDegrees: self.locationManager.travelHeadingDegrees
            )
        }
    ...
    private func configureBindings() {
        ...
        navigationManager.$isNavigating
            ...
            .sink { [weak self] isNavigating in
                guard let self else { return }
                if !isNavigating, self.navigationManager.currentBoardCommand == .arrived {
                    self.routeStatusText = "Arrived at destination."
    ...
    private func updateEffectiveCommand() {
        ...
        boardBLEManager.send(command: BoardBLECommand(boardCommand: effectiveCommand))
        updateBannerPresentation()
    }
